...
export class ChatRoomComponent implements OnInit, OnDestroy, AfterViewChecked {
    ...
    subscribeToMessages(): void {
        ...
        this.signalRService.joinChatRoom(this.chatRoom.id).then(() => {
            ...
            this.signalRService.userKicked()
                .pipe(takeUntil(this.destroy$))
                .subscribe((data) => {
                    console.log(`👤 UserKicked subscription fired: ${data.username} був викинутий. Причина: ${data.reason}`);
                });

            this.signalRService.kicked()
                .pipe(takeUntil(this.destroy$))
                .subscribe((data) => {
                    alert(data.message);
            ...
            this.signalRService.chatRemoved()
                .pipe(takeUntil(this.destroy$))
                .subscribe((data) => {
                    console.log('Chat room removed, redirecting...');
                    this.router.navigate(['/chat']);
                });
    ...
    isSystemMessage(message: any): boolean {
        return message?.senderId === 'SYSTEM' || message?.senderUsername === 'System';
    }
